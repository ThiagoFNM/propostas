import { EmpresaRepository } from "./infra/repository/useEmpresaRepository.js";
import { LinhasMoveisRepository } from "./infra/repository/useLinhasMoveisRepository.js";
import { PlanoMovelRepository } from "./infra/repository/usePlanosMovelRepository.js";
import { FuncoesCalculoProposta } from "./application/useProposta.js";
...
const cnpj = "25267175000190"; 


export async function CalculoProposta() {
    const empresa = await empresaRepository.getByCnpj(cnpj);
    if (!empresa) return;
    ...
    const fatAtual = simulacao.reduce((acc, s) => acc + s.valorAtual, 0);
    const limiteInferior = simulacao.reduce((acc, s) => acc + s.limiteLinha, 0);

    const gapAlvo = fatAtual - limiteInferior;
    const percentualMaximoConta = fatAtual > 0 ? (gapAlvo / fatAtual) * 100 : 0;
